...
export const ModalAddExchange: React.FC = () => {
  const theme = useTheme();
  const { t } = useTranslation("common");
  const store = useUnit($modalAddExchange);
  const [checked, setChecked] = useState<string[]>([]);

  const handleClose = () => {
    onChangeModalAddExchange({ open: false });
  };
  ...
  return (
    ...
    >
      <DialogTitle sx={{ p: 0, mb: 3 }}>
        <Typography variant="h6">{t("Добавьте биржы")}</Typography>
      </DialogTitle>
      <IconButton
        aria-label="close"
        onClick={handleClose}
